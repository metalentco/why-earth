...
const DesireSlot = ({ index, desire, selectedIds, onClick, onSave }: Props) => {
  ...
  useEffect(() => {
    ...
    if (desire.image) {
      setImage(desire.image);
    } else {
      setImage("");
      generateImage(desire.label).then((image) => {
        setImage(image);
      });
    }
  }, [desire.label, desire.image]);
    ...
    <div ref={ref} className="w-full flex justify-center items-center">
      <div
        className={`w-40 h-40 md:w-60 md:h-60 border-2 flex flex-row justify-center items-center overflow-hidden rounded-md select-none relative transition-all duration-300 ${
          selectedIds.includes(desire.id)
            ? "border-[#5AC0E5]"
            : "border-gray-300"
        } cursor-pointer`}
        onMouseEnter={() => setIsMouseOver(true)}
        onMouseLeave={() => setIsMouseOver(false)}
        onMouseDown={() => onClick(desire.id)}
        {...bind}
      >
        {image ? (
          <img
